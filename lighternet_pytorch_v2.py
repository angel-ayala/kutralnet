...
# img_dims = (68, 68)
img_dims = (64, 64)
model_name = 'model_lighternet_v3.pth'
...
if must_train:
    ...
    x_train = normalize(x_train, (-1, 1)).astype(np.float32)
    # x_val = dt.preprocess(x_val)
    x_val = normalize(x_val, (-1, 1)).astype(np.float32)

    # summary
    print('x_train shape:', x_train.shape)
    print(x_train.shape[0], 'train samples', x_train.min(), x_train.max())
    print(y_train[y_train==1].shape[0], 'fire')
    print(y_train[y_train==0].shape[0], 'no_fire')

    print('x_val shape:', x_val.shape)
    print(x_val.shape[0], 'test samples', x_val.min(), x_val.max())
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.0001)#, eps=1e-7)#, eps=None)
...
if must_test:
    ...
    x_test = normalize(x_test, (-1, 1)).astype(np.float32)
